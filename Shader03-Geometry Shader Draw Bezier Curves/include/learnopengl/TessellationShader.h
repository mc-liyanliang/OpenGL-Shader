...
public:
    unsigned int ID;
    // constructor generates the TessellationShader on the fly
    // ------------------------------------------------------------------------
    TessellationShader(const char* vertexPath, const char* fragmentPath, const char* tessControlPath = nullptr, const char* tessEvaluationPath = nullptr)
    {
        ...
        try 
        {
            ...
            // if tessControl shader path is present, also load a tessControl shader
			...
			// if tessEvaluation shader path is present, also load a tessControl shader
			if(tessEvaluationPath != nullptr)
            {
                eShaderFile.open(tessEvaluationPath);
                std::stringstream eShaderStream;
                eShaderStream << eShaderFile.rdbuf();
                eShaderFile.close();
				tessEvaluationCode = eShaderStream.str();
            }
        }
        ...
        // if tessControl shader is given, compile tessControl shader
		...
		// if tessEvaluation shader is given, compile tessControl shader
		unsigned int tessEvaluation;
		if(tessEvaluationPath != nullptr)
		{
			const char * eShaderCode = tessEvaluationCode.c_str();
			tessEvaluation = glCreateShader(GL_TESS_EVALUATION_SHADER);
			glShaderSource(tessEvaluation, 1, &eShaderCode, NULL);
